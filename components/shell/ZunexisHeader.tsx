...
import { Search, Play, Loader2, Command, Cpu } from "lucide-react";
...
export default function ZunexisHeader() {
  ...
  return (
    <header className="sticky top-0 z-[100] w-full h-14 flex items-center justify-between px-4 pt-1 glass-header select-none">
      {/* Branding */}
      ...
      <div className="flex-1 max-w-lg px-6">
        <button
          ...
          className="group flex h-9 w-full items-center justify-between gap-3 rounded-full 
               ...
               transition-all px-4 text-muted-foreground/60
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          className={cn(
            "h-7.5 min-w-22 px-4 rounded-full transition-all duration-200 font-bold text-[10px] tracking-[0.05em] uppercase shadow-none",
            isRunning
